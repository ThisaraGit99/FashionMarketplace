...
const CartDrawer = ({ open, setOpen }: CartDrawerProps) => {
  ...
  return (
    <div className={`fixed top-0 right-0 h-full w-full md:w-96 bg-white shadow-lg transform transition-transform duration-300 z-50 flex flex-col ${open ? '' : 'translate-x-full'}`}>
      <div className="p-4 border-b border-gray-200 flex justify-between items-center">
        <h2 className="text-xl font-semibold font-poppins">Your Cart ({totalItems})</h2>
        <Button variant="ghost" size="icon" onClick={() => setOpen(false)}>
          <span className="sr-only">Close panel</span>
          <i className="fas fa-times text-gray-500 hover:text-gray-700" />
        </Button>
      </div>
      ...
      {cart.length > 0 && (
        <div className="p-4 border-t border-gray-200">
          ...
          <Button
            asChild
            className="w-full bg-secondary hover:bg-secondary/90 text-white py-3 rounded font-medium mb-2"
          >
            <Link href="/checkout">Proceed to Checkout</Link>
          </Button>
          <Button 
            variant="outline" 
            className="w-full border border-primary text-primary hover:bg-primary hover:text-white py-2 rounded font-medium transition"
            onClick={() => setOpen(false)}
          >
            Continue Shopping
          </Button>
